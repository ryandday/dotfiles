return {
  ...
  -- Enhanced diff viewer
  {
    ...
    config = function()
      ...
      -- Auto-refresh diffview when files change
      local diffview_group = vim.api.nvim_create_augroup("DiffviewAutoRefresh", { clear = true })
      
      -- Refresh on file save
      vim.api.nvim_create_autocmd("BufWritePost", {
        group = diffview_group,
        callback = function()
          -- Only refresh if diffview is open
          if vim.fn.exists(":DiffviewRefresh") == 2 then
            local diffview_open = false
            -- Check if any buffer has the diffview filetype
      ...
      -- Refresh on focus gained (when switching back to nvim)
      ...
      -- Timer-based auto-refresh every few seconds
      ...
      local function start_auto_refresh()
        ...
        refresh_timer:start(3000, 3000, vim.schedule_wrap(function()
          -- Check if diffview is still open
          local diffview_open = false
          for _, buf in ipairs(vim.api.nvim_list_bufs()) do
            if vim.api.nvim_buf_is_loaded(buf) then
              local ft = vim.api.nvim_buf_get_option(buf, 'filetype')
          ...
          if diffview_open and vim.fn.exists(":DiffviewRefresh") == 2 then
            vim.cmd("DiffviewRefresh")
          else
            -- Stop timer if diffview is closed
            if refresh_timer then
              refresh_timer:stop()
              refresh_timer = nil
            end
      ...
      -- Start auto-refresh when diffview opens
      ...
      -- Stop auto-refresh when diffview closes
      vim.api.nvim_create_autocmd("BufDelete", {
        ...
        callback = function(args)
          ...
          if vim.api.nvim_buf_is_valid(buf) then
            local ft = vim.api.nvim_buf_get_option(buf, 'filetype')
            if ft == 'DiffviewFiles' or ft == 'DiffviewFileHistory' then
              -- Check if this was the last diffview buffer
              vim.schedule(function()
                local still_open = false
                for _, check_buf in ipairs(vim.api.nvim_list_bufs()) do
                  if check_buf ~= buf and vim.api.nvim_buf_is_loaded(check_buf) then
                    local check_ft = vim.api.nvim_buf_get_option(check_buf, 'filetype')
                    if check_ft == 'DiffviewFiles' or check_ft == 'DiffviewFileHistory' then
                      still_open = true
                      break
                    end
                  end
